...
const MODEL_DATA = {
  'claude-haiku': {
    ...
    inputRatio: 0.20,
    systemOverhead: 2000,          // Anthropic hidden system prompt overhead, tokens
    gpuWatts: 400,                 // NVIDIA A100 SXM4 TDP
    speed: 'Very Fast',
  ...
  'claude-sonnet': {
    ...
    inputRatio: 0.20,
    systemOverhead: 2000,
    gpuWatts: 700,                 // NVIDIA H100 SXM5 TDP
    speed: 'Fast',
  ...
  'gpt-4o-mini': {
    ...
    inputRatio: 0.20,
    systemOverhead: 1500,          // OpenAI system prompt overhead estimate
    gpuWatts: 400,
    speed: 'Very Fast',
...
const VA = {
  ...
  waterPerKwh: 1.8,            // L/kWh — on-site WUE, Northern VA data centers (Li et al. 2023)
  upstreamWaterPerKwh: 0.5,    // L/kWh — upstream grid water consumption, VA mix (EPRI 2011 / NREL 2020)
  pue: 1.45,                   // Power Usage Effectiveness — JLARC 2024 Northern VA average
...
function estimateTokens(text) {
  ...
  // Chain-of-thought reasoning — model produces reasoning trace before answer (~50% more tokens)
  const hasChainOfThought = /\b(think step.?by.?step|let'?s think|reason through|show your (work|reasoning|thinking|steps)|explain your reasoning|walk me through|think through|reason step.?by.?step|think (carefully|deeply|aloud|out loud)|work through this|before (you answer|answering|responding))\b/i.test(lower);

  // Table format — Markdown tables are token-expensive (pipes, padding, header row)
  const hasTableFormat = /\b(in a table|as a table|in tabular form|formatted? as a (table|spreadsheet)|create a? table|make a? table|table format|comparison table|with a table)\b/i.test(lower);
  ...
  if (hasTableFormat)    addons += 220;  // Markdown table syntax overhead
  ...
  // Chain-of-thought: model emits a reasoning trace before the final answer (~50% more tokens)
  if (hasChainOfThought) {
    estimatedOutput = Math.round(estimatedOutput * 1.5);
    outputReason += ' · chain-of-thought';
  }
...
function calculateImpact(modelKey, inputTokens, outputTokens) {
  ...
  // Add hidden system prompt overhead to input (users don't see this but models process it)
  const effectiveInput = inputTokens + (m.systemOverhead || 0);
  const weightedTokens = effectiveInput * m.inputRatio + outputTokens;
  // Chip-level inference energy
  const inferenceKwh = weightedTokens * m.energyPerOutputToken;
  // Total data center energy: multiply by PUE (cooling, power conversion, networking overhead)
  const energyKwh = inferenceKwh * VA.pue;
  // Water: on-site evaporative cooling (WUE) + upstream grid water to generate electricity
  const waterMlOnsite   = energyKwh * VA.waterPerKwh * 1000;
  const waterMlUpstream = energyKwh * VA.upstreamWaterPerKwh * 1000;
  const waterMl = waterMlOnsite + waterMlUpstream;
  // Carbon from grid electricity
  const carbonMg = energyKwh * VA.gridCarbonIntensity * 1000;
  // GPU compute time (chip-level, not wall-clock latency)
  const gpuMs = (inferenceKwh * 3_600_000) / (m.gpuWatts || 400);
  return { energyKwh, inferenceKwh, waterMl, waterMlOnsite, waterMlUpstream, carbonMg, gpuMs };
}
